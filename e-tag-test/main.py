...
def run_rtt_verification():
    ...
    try:
        ...
        print("\n[Step 1] Cold Visit - Installing Service Worker")
        driver.get(TARGET_URL)
        time.sleep(5)

        # Capture TRUE Cold Visit timing
        ...
        print(f"Actual Cold Page Load: {cold_nav:.2f}ms")

        print("\n[Step 1.5] Warm-up - Caching Resources with ETags")
        driver.refresh()
        time.sleep(5)

        # [Crucial] Clear only standard cache.
        driver.execute_cdp_cmd('Network.clearBrowserCache', {})
        print("💡 Browser HTTP Cache cleared")
        ...
        print("\n[Step 2] Refresh - Measuring Optimized 0-RTT")
        driver.get(TARGET_URL)
        time.sleep(3)
        ...
        print(f"Optimized Refresh Page Load: {refresh_nav:.2f}ms")
        refresh_nav = driver.execute_script("return window.performance.getEntriesByType('navigation')[0].duration")
        timings["Refresh (Warm)"] = refresh_nav
