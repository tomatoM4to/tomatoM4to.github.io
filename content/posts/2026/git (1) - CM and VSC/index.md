...
* **Delta based VCS**: 파일과 그 파일에 가해진 변경 사항(Delta) 의 집합을 저장하는 방식, CVS 와 SVN 이 대표적
* **Snapshot based VCS**: 커밋할 때마다 변경된 파일을 복사하여 저장하는 방식, 변경되지 않은 파일은 이전 커밋에서 참조하는 방식으로 저장 공간을 절약, Git, Mercurial 이 대표적
