...
### Proxy Cache
학교나 기업 같은 기관의 네트워크 게이트웨이에 설치되는 Cache Server 로, 원본 Server 에겐 Client 처럼, Client 에겐 Server 처럼 동작하는 이중 역할을 수행한다.
...
**문제 상황**
* 링크 사용률이 약 `97%(1.50/1.54)`에 달해 큐잉 지연(Queueing delay)이 기하급수적으로 증가해 사용자의 체감 속도가 매우 느려짐

**해결책**
**Proxy Cache**를 도입하여 문제를 해결할 수 있다. 만약 Cache Hit Rate(캐시 적중률)가 `0.4(40%)`라고 가정해 보자.

Proxy Cache를 도입하면 전체 트래픽 중 `60%` 만이 **외부 접속 링크를** 타게 되므로, 링크 사용률은 다음과 같이 획기적으로 **감소**한다.

$$
\text{링크 사용률} = \frac{\text{트래픽} \times (1 - \text{Cache Hit Rate})}{\text{링크 대역폭}} = \frac{1.50 \times (1 - 0.4)}{1.54} \approx 0.58
$$

링크 사용률은 `58% (0.9/1.54)`로 감소하여, 큐잉 지연이 사실상 없어진다.

그렇다면 캐시 도입 후 **사용자가 체감하는 최종 평균 응답 시간(Expected Delay)** 은 어떻게 될까? 이는 아래의 공식으로 계산할 수 있다.

$$
E[D] = h * D_{hit} + (1-h) * D_{miss}
$$

* $h$ : Cache Hit Rate
* $D_{hit}$ : Cache Hit 시의 응답 시간, LAN 구간이므로 수 밀리초 수준으로 매우 짧음
* $D_{miss}$ : Cache Miss 시의 응답 시간, 원본 서버에서 가져옴
