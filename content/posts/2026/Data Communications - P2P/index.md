...
* $\frac{NF}{u_s + \sum_{i=1}^{N} u_i}$ : System 전체가 **총 다운해야 할 NF** 를 네트워크 **전체의 업로드 능력(Server + 모든 Peer 의 업로드 속도의 합) 으로 나눈 시간**
...
### Circular DHT
전체 Network 지도가 없는 상태에서 목적지를 찾아가는 Routing 방법중 가장 기본적이고 단순한 방법

각 Peer 는 오직 자신의 Predecessor 와 Successor 의 정보만 알고 있다, 목적이 ID 를 찾을 때까지 이웃에게 질문을 계속 옆으로 넘기는 (순차탐색) 방식으로 동작한다.

> 맨 끝 Peer 까지 가면 다시 처음 Peer 로 돌아오게 되는 Circular 구조이기 때문에 Circular DHT 라고 불림

### Finger Table / Shortcuts
기존 `O(N)` 탐색 비효율을 극복하기 위해 도입된 개념

Peer 는 자신의 Predecessor 와 Successor 뿐만 아니라, 건너편에 있는 Peer 들의 정보도 추가로 추적하고 유지한다. (이 목록을 Finger Table 이라고 부름)

결과적으로 `O(log N)` 시간에 목적지 Peer 를 찾을 수 있게 됨

### Peer Churn
Peer 들이 언제는 Network 에서 떠날수 있기 때문에, Circular DHT 고리를 유지하기 위한 메커니즘이 필요하다.

* Ping : 각 Peer 는 안정성을 위해 하나의 이웃이 아닌 두개 이상의 노드 들 장보들의 주소를 알고 있다, 그리고 주기적으로 Ping 을 보내 확인한다.
* 복구 : 만약 바로 다음 Node 가 떠나면 다음과 같으 복구한다.
  1. 예건대 4번 Peer 의 바로 다음인 5번 Peer 가 떠났다고 가정하자.
  2. 4번 Peer 는 5번의 이탈을 감지하고 자신의 알고 있던 두번째 후속자인 8번 Peer 를 새로운 Successor 로 설정한다.
  3. 4번 Peer 는 8번 의 Successor 정보를 추가로 업데이트 하여, 백업을 다시 채워 둔다.
