...
DFS 가 여러 Node에 걸쳐 Read, Write, Replication 과 같은 기능을 제공하는 시스템이라면, **Distributed Data Processing 은 여러 Node 에 걸쳐 저장된 파일들에 필터링, 검색, 통계 등 연산과 처리 기능을 제공하는 거대한 시스템**이다. 대표적으로 **Apache Spark (오픈소스)** 가 있다.

유투브에 비유하면 동영상 업로드와 시청은 DFS 의 기능에 해당하고, 동영상 검색, 추천, 통계 분석 등은 DDP 의 기능에 해당한다.

DDP 시스템은, 한 Node 에서 데이터를 처리하지 않고, 분산된 각 Node 에 Task 를 할당하여 병렬적으로 데이터를 처리하는 방식으로 동작한다. 이렇게 분산된 각 Node 에서 처리된 결과를 모아서 최종 결과를 만들어 낸다.
...
sequenceDiagram
    autonumber
    actor User as 유저
    participant DDP as DDP (Spark 마스터)
