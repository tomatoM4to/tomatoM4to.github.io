...
XML 이나 JSON 과 같이 구조화된 요소와 자유로운 텍스트가 혼합된 데이터 포맷을 말한다. **일종의 Rule 이 존재하는 데이터**라고 생각하면 된다. `JSON`, `XML` 이 대표적이고 `Graph=(V, E)` 도 이에 해당한다.
...
이러한 DFS 에는 **Replication** 이라는 기능을 조건적으로 제공해야 하는데, 가령 한 Node 가 화제나, 정전으로 멈췄을때, 파일이 손실되지 않도록 **같은 파일들을 두대 이상의 Node 에다가 복사하는 기능**이다.
...
5. **DDP 는** 각 Worker Node 들로부터 받은 결과를 모아서 최**종 결과를 만들어 유저에게 전달**
