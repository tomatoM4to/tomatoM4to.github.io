...
Dart 를 배우기 위해 제 블로그를 방문해 주셔서 감사합니다. 하지만 다들 Dart 언어에는 별로 관심이 없으실거 같아요. 왜냐면 대부분 Flutter 에 관심이 있지 Dart 에는 비교적 관심이 적으니까요, 그래도 Flutter 를 다루기 전 Dart 에 대해 익숙해 진다면 Flutter를 보다 쉽게 다룰수 있을겁니다. 이 Dart 강의는 Dart 에 대해 깊게 다루지 않습니다. 또한 프로그래밍 언어를 처음 배우는 사람을 대상으로 하는 강의도 아닙니다. Tour 정도 랄까요? Flutter 를 다루기 전 빠르게 Dart 언어에 익숙해 지는것에 목적을 두고 있습니다.

먼저 Dart 언어에 대해 알아보기 전 Dart 를 만든 팀에서 말하는 Dart 의 장점에 대해 알아보겠습니다.
...
JIT, AOT.. 같은 용어는 다음장에서 바로 다루겠습니다. 일단 중요한점은 <U>Dart 는 컴파일러 언어</U>이며, <G>Flutter 는 Dart 로 작성된 UI 툴킷</G>이란걸 알수 있습니다. 그리고 Dart 는 여러 플랫폼에서 실행 가능하도록 컴파일 할 수 있다는것도 알수 있네요.

그리고 또 한가지로, **밴치마크**에 대해서 궁금하실수 있을거 같은데요, 사실 언어의 **밴치마크**는 그렇게 중요한 개념은 아니긴 한데.. 일례로 Python 같은 경우도 Numba, Numpy 등등 C 로 작성된 라이브러리를 사용하면 Java, C++ 만큼의 성능이 나오기도 하니까요, 하지만 그렇다고 해도 프로그래밍 언어라는 도구의 기본적인 성능을 완전히 무시하기도 좀 힘들긴 하죠, 하지만 Dart 의 성능을 정밀하게 측정한 자료는 찾지 못했습니다. 그래서 <U>여러 커뮤니티를 돌아다니면서 종합한 결과 Dart 의 성능은 Java 와 비슷한 성능</U>이 나온다고 사람들이 생각하는거 같더라구요. 그래서 Dart 는 Java 와 비슷한 성능을 낸다고 생각하시면 될거 같습니다.
