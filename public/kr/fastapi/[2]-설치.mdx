...
Importing from C:\Users\b_beginner\app
module 🐍 main.py

# FastAPI가 import할 모듈과 그 안의 app 객체를 확정
code Importing the FastAPI app object from the module with the following code: from main import app

# uvicorn-스타일 import 경로(모듈:변수)로 치환한 결과
...
INFO Will watch for changes in these directories: ['C:\\Users\\b_beginner']

# 실제 ASGI 서버(uvicorn)가 기동 완료. Ctrl+C 로 전체 프로세스를 종료 가능
INFO Uvicorn running on http://127.0.0.1:8000 (Press CTRL+C to quit)

# 코드 변경 감시용 리로더 프로세스의 PID가 19120임을 표시
INFO Started reloader process [19120] using WatchFiles

# 실제 요청을 처리할 메인 uvicorn 워커 PID가 7736
