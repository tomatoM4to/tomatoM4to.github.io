...
## Router
Router는 말 그대로 길을 찾아주는 역할을 합니다. 여러 Router중 어떤 Router를 통해 패킷을 보낼지 결정하는걸 어떤 문을 열지 결정하는것처럼 보인다고 Gateway 라 부르기도 합니다.

Bridge 처럼 Broadcast 를 차단하여 Network를 분리하고 하며, 논리적으로 분리된 둘 이상의 Network를 연결하기도 하는 역할을 수행하게 됩니다.

가장 중요한 점은 각 Router은 패킷의 최적 경로를 찾기 위한 Routing Table를 가지고 있습니다. 이 Routing Table은 각 Router가 가지고 있는 작은 DB라 볼수 있습니다.
